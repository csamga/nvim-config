local on_attach = function(client, bufnr)
   ...
   vim.keymap.set('n', '<leader>dk', function()
      vim.diagnostic.jump({ count = -1 })
   end, { buffer = bufnr, desc = 'Jump to prev diagnostic' })
   ...
   vim.keymap.set('n', '<leader>f', format_callback, {
   ...
   end, { buffer = bufnr, desc = 'All buffers diagnostics' })

   -- List/jump LSP definitions
   vim.keymap.set('n', '<leader>df', function()
      builtin.lsp_definitions({ trim_text = true })
   end, { buffer = bufnr, desc = 'LSP definitions' })

   -- List/jump LSP references
